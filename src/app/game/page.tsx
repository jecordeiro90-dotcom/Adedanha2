...
export default function GamePage() {
  ...
  const handleStop = async () => {
    ...
    try {
        ...
        // The first person to stop becomes the round winner and initiates the end of the round.
        if (!game?.roundWinner) {
        ...
        // After submitting, player goes into a waiting phase
        setPhase('WAITING');
    } catch(e) {
        console.error("Failed to submit answers:", e);
        toast({ title: "Erro", description: "Não foi possível enviar suas respostas.", variant: "destructive" });
  ...
  const renderContent = () => {
    ...
    // This handles the case where the player has submitted but is waiting for others.
    ...
    <div className="flex justify-center items-start min-h-screen p-4 sm:p-8">
      ...
      <Card className="w-full max-w-4xl shadow-2xl animate-in fade-in-50 zoom-in-95 duration-500">
        <CardHeader className="text-center relative">
          ...
          {game?.currentLetter && <div className="absolute top-4 right-4 bg-primary text-primary-foreground rounded-full w-16 h-16 flex items-center justify-center text-4xl font-extrabold border-4 border-background shadow-lg">{game.currentLetter}</div>}
        </CardHeader>
